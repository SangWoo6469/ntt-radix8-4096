...
echo "BFU8 곱셈 레이어만 병렬처리 벤치마크 시작..."
...
# Barrett + BFU_MULT_PAR4=0 (곱셈 비병렬처리)
for t in "${TPB_LIST[@]}"; do
  run_case "barrett_bfu8_sequential" "$t" "-DTILES_PER_BLOCK_S01=$t -DTILES_PER_BLOCK_S23=$t -DBFU_MULT_PAR4=0"
done

# Montgomery + BFU_MULT_PAR4=0 (곱셈 비병렬처리, 상수 인코딩 포함)
for t in "${TPB_LIST[@]}"; do
  run_case "montgomery_bfu8_sequential" "$t" "-DTILES_PER_BLOCK_S01=$t -DTILES_PER_BLOCK_S23=$t -DUSE_MONTGOMERY=1 -DMONT_CONST_ENCODE=1 -DBFU_MULT_PAR4=0"
done

echo "BFU8 곱셈 레이어만 병렬처리 벤치마크 완료!"
