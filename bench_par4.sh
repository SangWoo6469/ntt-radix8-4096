...
# Barrett + BFU_MULT_PAR4
for t in "${TPB_LIST[@]}"; do
  run_case "barrett_par4" "$t" "-DTILES_PER_BLOCK_S01=$t -DTILES_PER_BLOCK_S23=$t -DBFU_MULT_PAR4=1"
done

# Montgomery + BFU_MULT_PAR4 (상수 인코딩 포함)
for t in "${TPB_LIST[@]}"; do
  run_case "montgomery_par4" "$t" "-DTILES_PER_BLOCK_S01=$t -DTILES_PER_BLOCK_S23=$t -DUSE_MONTGOMERY=1 -DMONT_CONST_ENCODE=1 -DBFU_MULT_PAR4=1"
done
