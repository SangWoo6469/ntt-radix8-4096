...
# Barrett + BFU8 전체 병렬 처리
for t in "${TPB_LIST[@]}"; do
  run_case "barrett_bfu8_full_parallel" "$t" "-DTILES_PER_BLOCK_S01=$t -DTILES_PER_BLOCK_S23=$t -DBFU_FULL_PARALLEL=1"
done

# Montgomery + BFU8 전체 병렬 처리 (상수 인코딩 포함)
for t in "${TPB_LIST[@]}"; do
  run_case "montgomery_bfu8_full_parallel" "$t" "-DTILES_PER_BLOCK_S01=$t -DTILES_PER_BLOCK_S23=$t -DUSE_MONTGOMERY=1 -DMONT_CONST_ENCODE=1 -DBFU_FULL_PARALLEL=1"
done

echo "BFU8 전체 병렬 처리 벤치마크 완료!"